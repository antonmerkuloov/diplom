...
export default function InventoryList({ characterId }: { characterId: string }) {
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg">
      ...
      {items.length === 0 ? (
        <p className="text-gray-400 mt-2">Инвентарь пуст</p>
      ) : (
        <ul className="mt-2 space-y-1">
          {items.map(item => (
            <li key={item.id} className="flex justify-between items-center">
              ...
              <div>
                <button onClick={() => toggle(item.id, item.equipped)} className={`text-xs px-2 rounded ${item.equipped ? 'bg-gold text-black' : 'bg-gray-600'}`}>
